...
| TypeScript game module | Weather, movement, racing line, wear, fuel, grip, traffic, pits, incidents, penalties, scoring | OpenAI credentials or prose generation |
...
App.tsx owns route-level session restoration and transitions. RaceScene keeps the high-frequency mutable RaceSnapshot in a ref so animation frames do not force a full React render. It publishes throttled cloned snapshots to the HUD.
