export function AISystemVisual({ system, active = false, metric = '68%' }: { system: 'ML' | 'DL' | 'RL'; active?: boolean; metric?: string }) {
  if (system === 'ML') {
    return <svg className={'ai-system-visual ml' + (active ? ' active' : '')} viewBox="0 0 84 44" role="img" aria-label="Prediction curve and confidence range"><path className="range" d="M4 32 C19 30 26 25 38 23 S61 8 80 7 L80 19 C61 16 54 25 39 29 S17 38 4 39 Z" /><path className="line" d="M4 35 C19 33 27 27 39 26 S61 13 80 12" /><circle cx="39" cy="26" r="3" /><text x="5" y="10">{metric}</text></svg>
  }
  if (system === 'DL') {
